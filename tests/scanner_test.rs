...
fn test_scanner_with_custom_patterns() {
    let patterns = vec![
        ("test".to_string(), regex::Regex::new(r"test").unwrap())
    ];
...
fn test_scan_respects_gitignore() {
    ...
    fs::write(temp_path.join("normal_file.txt"), "ghp_1234567890abcdefghijklmnopqrstuvwxyz").unwrap();
...
fn test_scan_nested_directories() {
    ...
    fs::write(nested_dir.join("config.rs"), "let api_key = \"AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI\";").unwrap();
